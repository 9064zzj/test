...
public class AI_12 {
    ...
    public String fileName = "report2";
...
//        public Set<Vertex> Vertices = new HashSet<>();
    ...
    public boolean PathFinder(Vertex Start,int TargetNum){
        ...
        LinkStack.push(starter);
        PosStack.push(starterPos);
        
        while(!LinkStack.isEmpty()){
            Node LinkStatus = LinkStack.pop();
            Pos PosStatus = PosStack.pop();
            ...
            for(int i=0; i< CurrentPos.Adjacent.size();i++){
                ...
                if(!Reached_Node.contains(ChildNode) && !LinkStack.contains(ChildNode)){
                         if(ChildNode.complete(TargetNum)){
                             if(ChildPos.get(ChildPos.size()-1).equals(Start))
                              ...
                              LinkStack.push(ChildNode);
                              PosStack.push(ChildPos);
                         }
                }
            } 
        }
